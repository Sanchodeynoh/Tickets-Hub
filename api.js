// api.js — Frontend to Backend connector
// Change the URL below to your Render backend URL

const API_URL = 'https://tickets-hub-api.onrender.com';
...
const Auth = {
  ...
  async login(email, password) {
    ...
    // SAVE ADMIN DATA FOR GREETING IN admin.html
...
// EVENTS
const Events = {
  async getAll() { return apiFetch('/events'); },
  async getOne(id) { return apiFetch('/events/' + id); },
  async create(payload) { return apiFetch('/events', { method: 'POST', body: JSON.stringify(payload) }); },
  async update(id, payload) { return apiFetch('/events/' + id, { method: 'PUT', body: JSON.stringify(payload) }); },
  async delete(id) { return apiFetch('/events/' + id, { method: 'DELETE' }); }
};
...
const Orders = {
  ...
  async getMy() { return apiFetch('/orders/my'); },
  async getOne(orderId) { return apiFetch('/orders/' + orderId); },
